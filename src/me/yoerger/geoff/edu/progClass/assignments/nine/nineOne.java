...
public class nineOne implements Analysis {
	...
	public static void main(final String[] args) throws ImageFormatException {
		...
		// Picture greenScreenedPic = new
		// Picture("P:\\Programming\\Workspace\\ProgrammingClass\\util\\Maze_Background.jpg");
		...
		// Picture backgroundPic = new
		// Picture("P:\\Programming\\Workspace\\ProgrammingClass\\util\\BeauPicture.jpg");
		Preconditions.checkArgument(
				greenScreenedPic.getHeight() <= backgroundPic.getHeight(),
				"Background picture has to small height.");
		Preconditions.checkArgument(
				greenScreenedPic.getWidth() <= backgroundPic.getWidth(),
				"Background picture has to small width.");
		Picture combinedPicture = new Picture(backgroundPic.getWidth(),
				backgroundPic.getHeight());
		...
		int limitLeft = (int) trunc((backgroundPic.getWidth() - greenScreenedPic
				.getWidth()) / 2);
		...
		System.out.printf("Using color as chroma key (R,G,B): %1$s,%2$s,%3$s",
				highestColor.getRed(), highestColor.getGreen(),
				highestColor.getBlue());
		...
		for (Pixel combinedPixel : combinedPicture.getPixels()) {
			...
			try {
				Pixel appliedPixel = greenScreenedPic.getPixel(x - limitLeft, y
						- limitTop);
			...
			// if(x < limitLeft || x > limitRight || y > limitBottom || y <
			// limitTop) {
			// combinedPixel.setColor(backgroundPic.getPixel(x, y).getColor());
			// } else {
			// if (greenScreenedPic.getPixel(x - limitLeft, y -
			// limitTop).getColor().equals(highestColor)){
			// combinedPixel.setColor(backgroundPic.getPixel(x, y).getColor());
			// i++;
			// } else {
			// combinedPixel.setColor(greenScreenedPic.getPixel(x - limitLeft, y
			// - limitTop).getColor());
